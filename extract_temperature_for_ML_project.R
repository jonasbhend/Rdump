...
t2m <- dwhget(stats = 'all', mstypes=NULL, pars=param, t.beg="2015.10.01", t.end="2015.10.31")
stations <- na.omit(unique(t2m$station))
...
# param.meta <- dwh.para.info(pars = param, language = 'E')
...
## NetCDF Dimensions
time.nc <- ncdim_def("time", "hours since 2015-10-01 0:0", 1:nrow(t2m.wide) - 1, unlim=TRUE, longname = 'time of measurement')
...
vars.nc$t2m <- ncvar_def("t2m", "deg. C", list(station.nc, time.nc), missval=-1e20)


## open file and write to it
ncfile <- paste0("/store/msclim/bhendj/ml_ce_zhang/full/observations/meteoswiss_t2m_20151001-", format(yesterday, "%Y%m%d"),".nc")
