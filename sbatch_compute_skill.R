...
FairBss <- function(ens, ens.ref, obs, ...){
  stopifnot(is.matrix(ens), dim(ens) == dim(obs))
  mm <- sapply(1:3, function(i){
    ensi <- cbind(rowSums(ens[,-i]), ens[,i])
    ens.refi <- cbind(rowSums(ens.ref[,-i]), ens.ref[,i])
    obsi <- cbind(rowSums(obs[,-i]), obs[,i])
    1 - mean(FairRps(ensi, obsi)) / mean(EnsRps(ens.refi, obsi))
  })
  names(mm) <- paste0('cat', seq(along=mm))
  return(mm)
}
...
scorelist <- list(Ens2AFC='generalized discrimination score',
                  ...
                  FairBss.cat1='fair Brier skill score (lower tercile)',
                  FairBss.cat2='fair Brier skill score (middle tercile)',
                  FairBss.cat3='fair Brier skill score (upper tercile)',
...
scorefun <- function(f, o, score, prob=NULL, ...){
  if (length(grep('Rps', score)) == 1 | score == 'EnsRocss' | score == 'EnsRoca' | score == 'FairBss' ){
    ## do not pass on strategy as long as this is not doing a full out-of-sample protocol
    return(veriApply(verifun=score, fcst=f, obs=o, ensdim=4, tdim=5, prob=prob))
  } else {
    return(veriApply(verifun=score, fcst=f, obs=o, ensdim=4, tdim=5, prob=NULL, ...))
  }
}
...
if (length(args) == 0){
  args <- c('UCAN-PP-AN1-30PC',
            'WFDEI',
            'pr',
            'EAF-22',
            'none-crossval1_1991-2012_WFDEI',
            '05',
            'standard',
            TRUE,
            'none',
...
} else if (whatscores == 'special') {
  scores <- c("FairBss", "FairRpss", "EnsCorr", "EnsMe", "EnsMsess", "EnsRocss")
}
...
## dirty hack for paper for WP21
fcfiles <- fcfiles[1:min(20, length(fcfiles))]
...
nens <- fdims[[which(!fdimnames %in% grep('tim|lon|lat|ncells|nb2|bnd', fdimnames, value=TRUE))]]$len
...
ncomplete <- sum(as.numeric(format(fctime, '%d')) >= 14)
nc_close(nc)
...
is.in.obs <- sapply(fc.times, function(x) all(format(x[1:ncomplete], '%Y%m') %in% format(obstime, '%Y%m')))
...
for (seasonal in seasonals){
  ...
  if (seasonal){
    ...
    if (grid == 'EAF-22'){
      ...
      obs.seas[nrow(obs.seas),,,] <- colMeans(obs[mii,,,,drop=F], dims=1)
    }
  ...
  for (detrend in detrends){
    ...
    if (detrend){
      ...
      obs.seas <- aperm(apply(obs.seas, 1:3, function(y){
        yna <- !is.na(y)
        if (any(yna)){
          ymask <- rep(NA, length(y))
          ymask[yna] <- seq(1,sum(yna))
          mean(y) + lm(y ~ seq(along=y))$res[ymask]
        } else {
          NA*y
        }
      }), c(2,3,4,1))
...
#       } ## end of loop on longitudes
    ...
    for (score in scores){
      if (score == 'EnsRocss' | score == "FairBss" ){
        for (i in 1:3) {
      ...
      if (is.list(sfo)){
        ...
        if (score == 'EnsRocss' | score == 'FairBss' ){
          for (i in 1:3){
            ...
            if (score == 'EnsRocss') ncvar_put(ncout, varid=paste0(score, '.cat', i, '.sigma'), vals=aperm(sfo[[paste0('cat', i, '.sigma')]], c(2,3,1)))
          }
        } else if (score == 'CRPSdecomp') {
          for (nn in names(sfo)){
